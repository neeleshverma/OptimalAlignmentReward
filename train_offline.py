...
from agents import iql
from agents.reward_labelling import rewarder as rewarder_lib

_CONFIG = config_flags.DEFINE_config_file("config", "configs/iql_mujoco.py")
_WORKDIR = flags.DEFINE_string('workdir', '/tmp/ot', '')
...
def get_demonstration_dataset(config):
  ...
  if config.use_dataset_reward:
    ...
    if config.squashing_fn == 'linear':
      ...
      # TODO: Make config key required for OptimalTransport
    ...
    rewarder = rewarder_lib.OptimalTransportRewarder(
